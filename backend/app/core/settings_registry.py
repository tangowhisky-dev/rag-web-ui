...
_APP_ONLY = [
    ...
    SettingDef("MARKDOWN_ENGINE", "System", "Markdown conversion engine",
...
_ORG_OVERRIDABLE = [
    ...
    # Utility Model — falls back to Response Model
    SettingDef("UTILITY_API_BASE", "Utility Model", "Utility API base URL",
               "str", None, scope="org", reload="next_request",
               description="Base URL for utility tasks (synonyms, evaluation, compaction, extraction). Falls back to OPENAI_API_BASE."),
    SettingDef("UTILITY_API_KEY", "Utility Model", "Utility API key",
               "str", None, scope="org", reload="next_request", secret=True,
               description="API key for utility tasks. Falls back to OPENAI_API_KEY."),
    SettingDef("UTILITY_MODEL", "Utility Model", "Utility model",
               "str", None, scope="org", reload="next_request",
               description="Smaller/faster model for utility tasks (synonyms, evaluation, compaction, extraction). Falls back to OPENAI_MODEL when unset.",
               model_picker=True, api_base_ref="UTILITY_API_BASE", api_key_ref="UTILITY_API_KEY"),
    ...
    SettingDef("DEDUP_SEMANTIC_THRESHOLD", "Retrieval", "Semantic dedup threshold",
               ...
               description="Cosine similarity above which chunks from different documents "
                           ...
                           "file_modified_at document is kept. 1.0 = disabled."),
    ...
    SettingDef("RETRIEVAL_SUBAGENT_MAX_ITERATIONS", "Agentic", "Retrieval sub-agent max iterations",
               "int", 4, scope="org", reload="next_request", min_value=1, max_value=10,
               description="Max think→tool rounds per retrieval sub-agent."),
    ...
    SettingDef("AGENT_TOTAL_TOOL_BUDGET", "Agentic", "Total tool-call budget",
               ...
               description="Total tool calls across all tools per turn for the main agent and sub-agents. When reached, the agent is forced to finalize."),
               "int", 25, scope="org", reload="next_request", min_value=1,
    ...
    SettingDef("AGENT_MAX_SEARCH_SPARSE", "Agentic", "Max search_sparse calls",
               "int", 5, scope="org", reload="next_request", min_value=0,
               description="Cap on search_sparse (SPLADE) tool calls per turn."),
    SettingDef("AGENT_MAX_SEARCH_DENSE", "Agentic", "Max search_dense calls",
               "int", 5, scope="org", reload="next_request", min_value=0,
               description="Cap on search_dense (semantic vector) tool calls per turn."),
    SettingDef("AGENT_MAX_RERANK", "Agentic", "Max rerank_results calls",
               "int", 5, scope="org", reload="next_request", min_value=0,
               description="Cap on rerank_results (cross-encoder) tool calls per turn."),
    SettingDef("AGENT_MAX_GRAPH_EXPAND", "Agentic", "Max graph_expand calls",
               "int", 3, scope="org", reload="next_request", min_value=0,
               description="Cap on graph_expand (Neo4j) tool calls per turn."),
    SettingDef("AGENT_MAX_KB_SEARCH", "Agentic", "Max kb_search_documents calls",
               "int", 10, scope="org", reload="next_request", min_value=0,
               description="Cap on kb_search_documents tool calls per turn."),
    SettingDef("AGENT_MAX_EXTRACT_DATA", "Agentic", "Max extract_data calls",
               "int", 5, scope="org", reload="next_request", min_value=0,
               description="Cap on extract_data tool calls per turn."),
    SettingDef("AGENT_MAX_CHART_GENERATE", "Agentic", "Max chart_generate calls",
               "int", 3, scope="org", reload="next_request", min_value=0,
               description="Cap on chart_generate tool calls per turn."),
    # Office document generation
    SettingDef("AGENT_MAX_OFFICE_LOAD_SKILL", "Agentic", "Max office_load_skill calls",
               "int", 1, scope="org", reload="next_request", min_value=0,
               description="Cap on office_load_skill calls per turn. Loading the same skill twice is wasteful."),
    SettingDef("AGENT_MAX_OFFICE_GENERATE", "Agentic", "Max office_generate calls",
               "int", 3, scope="org", reload="next_request", min_value=0,
               description="Cap on office_generate tool calls per turn."),
    SettingDef("AGENT_MAX_OFFICE_INSPECT", "Agentic", "Max office_inspect calls",
               "int", 15, scope="org", reload="next_request", min_value=0,
               description="Cap on office_inspect tool calls per turn. Allows multiple QA passes."),
    SettingDef("AGENT_MAX_OFFICE_EDIT", "Agentic", "Max office_edit calls",
               "int", 10, scope="org", reload="next_request", min_value=0,
               description="Cap on office_edit tool calls per turn."),
    ...
    SettingDef("OFFICECLI_VISUAL_QA", "Agentic", "OfficeCLI visual QA",
               "bool", True, scope="org", reload="next_request",
               description="Enable visual QA via screenshot rendering when the LLM supports vision."),
    SettingDef("OFFICECLI_WORK_DIR", "Agentic", "OfficeCLI work directory",
               "str", "/tmp/officecli", scope="app", reload="next_request",
               description="Temp directory for OfficeCLI document generation."),
    ...
    SettingDef("TOOL_CALL_TEMPERATURE", "Agentic", "Tool-call temperature",
               "float", 0.7, scope="org", reload="next_request", min_value=0.0, max_value=2.0,
               description="Temperature for tool selection, structured output, code, charts, and answer grading. Lower = more deterministic; 0.0 causes looping in small local models."),
    SettingDef("GENERATION_TEMPERATURE", "Agentic", "Answer generation temperature",
               ...
               description="Temperature for prose generation (final answers, document content, plans). Higher = more creative; avoids repetitive looping in local models."),
    ...
    SettingDef("CONTEXT_TOOL_BUDGET", "Context", "Tool context budget",
               "int", 16384, scope="org", reload="next_request", min_value=512,
    ...
    SettingDef("PROCESSING_TIMEOUT_SILENCE_S", "Ingestion", "Processing silence timeout (s)",
               "int", 600, scope="app", reload="next_request", min_value=10,
