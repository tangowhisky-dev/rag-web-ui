"""SPLADE sparse vector search tool — keyword matching across content."""
...
from app.services.retrieval.retrieval import sparse_search_docs
...
class SearchSparseInput(BaseModel):
    query: str = Field(description="Search query — keyword matching across document content.")
...
class SearchSparseTool(BaseAgentTool):
    ...
    async def _execute(self, input_obj: SearchSparseInput) -> dict:
        ctx = self.ctx
        if ctx is None:
        ...
        if not kb_ids:
            return {"ok": True, "result": {"hits": [], "query_used": input_obj.query, "search_type": "sparse", "count": 0}, "error": None, "tokens": 0, "terminate": False}
        ...
        # Synonym expansion (Redis-cached) — sparse benefits from keyword variants
        query, extra_queries = await expand_synonyms(input_obj.query, ctx)

        min_score = get_setting(ctx.db, "SPARSE_MIN_SCORE", ctx.org_id)

        try:
            docs = sparse_search_docs(
                query=query,
                kb_ids=kb_ids,
            ...
            logger.warning("[search_sparse] failed: %s", exc)
        ...
        for doc in docs:
            ...
            hit = {
                ...
                "citation_ref": {
                    ...
                    "source_tool": "search_sparse",
                    "citation_id": "",
                },
            }
            hits.append(hit)

        write_audit(ctx, "search_sparse", input_obj.model_dump(),
