"""Runner for the enterprise agent loop graph (v1 — DISABLED).

This file is commented out. The v2 pipeline (agent_runner_v2.py) is the
active pipeline. v1 code is retained for reference only.

To re-enable v1: uncomment this file and set USE_AGENTIC_V1=true in the
settings registry. But v1 is not maintained — it may not work with current
shared modules (helpers.py, tooling.py, observations.py, etc.) which have
been updated for v2 compatibility.
"""
...
#         eval_kwargs: Pre-resolved LLM kwargs (api_base, api_key, utility_model).
#     """
#     try:
