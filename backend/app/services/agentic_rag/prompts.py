...
- semantic_search: semantic vector search. Best for conceptual, natural-language, paraphrased, and meaning-based questions. Use when relevant documents may not share the user's exact wording. Default first choice for most questions. Results are cross-encoder reranked and soft-elbow filtered before returning.
...
- file_extract_table: extract a table from CSV/Excel in an attached file.
...
- office_load_skill: load OfficeCLI design guidelines for the target format (pptx/docx/xlsx). Call BEFORE office_generate to get font sizes, color palettes, layout rules, and quality check criteria. Returns skill content as an observation. Only call once per query.
...
- Search results are already cross-encoder reranked and soft-elbow filtered. No separate rerank call needed.
