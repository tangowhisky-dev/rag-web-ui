       ...
       | `modified` | File content changes → routed through `_handle_file()` (re-ingestion) |
       ...
       > **Note:** The batch timer (5-minute interval) exists in the code for backward compatibility but is not used for event-driven processing. Changes are
       processed immediately after the 1s write-completion delay.
       >
       > **Note:** The `_flush_batch()` method exists and is called when `remove_folder()` is invoked (flushing pending changes on teardown), but is never
       triggered by a timer during normal event-driven operation.
       ...
       └── _refresh_file_count(datastore_id)  [called from _on_changes, after processing all changes]
             Counts files on disk and updates DataStore.last_scan_total_files
       ```
       ...
       **Scan all datastores** (batch operation — not exposed as a REST endpoint):
       ...
       The frontend polls `GET /api/admin/datastores`:
       ...
       │   - scan_progress (from active_scans when manual scan is running)
       │
       ├── Polling frequency:
       │   - 2s when processing=true (event-driven processing active)
       │   - 2s when triggering=true (manual scan in progress)
       ...
       | Completed            | Green "Completed"                  | "N files / M processed"      | Scan button           |
       | Error                | Red "Error"                        | Error message                | Scan button           |
       ...
       and wants to process them immediately rather than waiting for the auto-scan interval.
       ---
       ...
       work without modification. This is needed because the original watchdog event object may have changed state (e.g., the file may have been deleted) by
       the time the 1s delay expires.
       ...
       invoked (flushing pending changes on datastore teardown). It is **never** called by a timer during normal event-driven operation — the batch timer was
       removed to avoid 5-minute delays.
