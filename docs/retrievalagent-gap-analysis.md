...
**our pipeline after this plan** (Phase 6 adds `extract_intent`):
```
load_context → expand_query → rewrite_query → extract_intent → plan
  → route_plan(clarify_interrupt|think)
  → think → route_think(tool|reflect_final)
  → tool → route_tool(reflect|reflect_final)
  → reflect → think
...
_run_relaxation_ladder:
  for each level (0,1,2):
    _run_retrieval_pass:
      ...
      merge_node (dedup + semantic dedup)
      optional metadata sort
      reranking_node (cross-encoder)
...
These become additional search terms in the exact and sparse legs (parallel BM25-style search). The original query is always preserved.
...
- `backend/app/services/retrieval/retrieval.py` — `exact_search_docs` and `sparse_search_docs` accept `extra_queries: list[str]` and run them in parallel, RRF-fusing results.
- `backend/app/services/agentic_rag/prompts.py` — add `SYNONYM_EXPANSION_PROMPT`.
- `backend/app/services/agentic_rag/graph_state.py` — add `synonyms`, `corrected_query`, `excluded_terms` to AgentState.

**New settings:**
- `SYNONYM_EXPANSION_ENABLED` (default true)
- `SYNONYM_VARIANTS` (default 3, max 5)

**Caching:** LLM synonym results should be cached per query string (same as retrievalagent's `_cache.aload("synonyms-v2", ...)`).
...
1. **Regex** (`extract_negation_terms` in `retrieval/intent_match.py`) — deterministic, fires in `_aprepare_node` before the synonym LLM. Covers DE/EN/FR/IT/ES/NL negation patterns: "not X", "but not X", "without X", "nicht X", "ohne X", "sans X", etc.
...
### Recommendation: **Implement — regex-only, no LLM**

The regex extractor is language-agnostic, deterministic, and adds zero latency. The LLM extractor is redundant for our use case (our queries are simpler than e-commerce product searches).

**Files to change:**
- `backend/app/services/agentic_rag/tools/rag_retrieve.py` — add `_extract_negation_terms(query)` function (port the regex from retrievalagent). Call it in `_run_retrieval_pass`. Post-filter merged docs in `merge_node` or after reranking.
- `backend/app/services/agentic_rag/nodes.py` — `merge_node` accepts `excluded_terms: list[str]` and drops matching docs.
...
- `backend/app/services/agentic_rag/kb_profile.py` (new file) — `profile_kb(org_id, kb_ids, db, redis) -> dict` function.
...
**API change:** None. The profile is computed lazily on first chat open and cached. No new endpoint needed.

**Frontend change:** None. The profile is backend-internal. The agent's improved context awareness will naturally produce better answers.
...
### Recommendation (user's idea): **Extend evaluation to include suggestion + retry_strategy, show as clickable suggestions**

This is a user-generated retry: instead of automatic retry (Phase 5, still skipped), the agent suggests follow-up queries and the user clicks one to retry.

**Backend changes:**
- `backend/app/services/agentic_rag/evaluator.py` — extend `AnswerEvaluation` with `suggestion: str`, `retry_strategy: str`, `follow_up_queries: list[str]`.
- `backend/app/services/agentic_rag/prompts.py` — extend `EVALUATION_SYSTEM_PROMPT` to ask for `suggestion`, `retry_strategy`, `follow_up_queries`.
- `backend/app/services/agentic_rag/nodes.py` — `answer_evaluation_node` extracts the new fields into state.
- `backend/app/services/agentic_rag/graph_state.py` — add `answer_suggestion`, `retry_strategy`, `follow_up_queries` to AgentState.
- `backend/app/services/agentic_rag/agent_graph/finalization.py` — `save_memory_node` saves the new fields to the `Message` row.
- `backend/app/models/chat.py` — add columns: `answer_suggestion (Text)`, `retry_strategy (String(20))`, `follow_up_queries (LONGTEXT, JSON)`.
- `backend/app/schemas/chat.py` — add fields to `MessageResponse`.

**Frontend changes:**
- `frontend/src/components/chat/answer.tsx` — render `follow_up_queries` as clickable `Suggestion` chips below the answer (using the existing `Suggestions`/`Suggestion` components from `ai-elements/suggestion.tsx`). Wire `onClick` to `onFollowUp`.
- `frontend/src/app/dashboard/chat/[id]/page.tsx` — pass `followUpQueries` from message to `AnswerCard`.
- `frontend/src/types/` — add `followUpQueries` to the message type.

**When to show suggestions:**
- Always show when `follow_up_queries` is non-empty.
- Show `suggestion` text (the reasoning) in the `ConfidenceCollapsible` section.
- Show `retry_strategy` as a label on the suggestion row ("Try a broader search:" / "Try a narrower search:" / "Look up this exact ID:").

**New settings:**
- `ANSWER_SUGGESTIONS_ENABLED` (default true)
- `MAX_FOLLOW_UP_QUERIES` (default 3)
...
**retrievalagent:** `_mmr_diverse(unique, lam=0.7, top_k=pool_size)` runs BEFORE reranking to remove near-duplicate docs from the candidate pool.
...
1. Port `extract_negation_terms` regex from retrievalagent's `retrieval/intent_match.py`.
2. Add `_extract_negation_terms(query) -> list[str]` to `rag_retrieve.py`.
3. Call it in `_run_retrieval_pass` before legs fire.
4. Post-filter merged docs in `merge_node` — drop docs whose `page_content` or `title` contains any excluded term.
5. Add `excluded_terms` to `AgentState`.
6. Unit test: "networking but not Linux" → excluded_terms=["Linux"], docs containing "Linux" are dropped.

**Files:**
- `backend/app/services/agentic_rag/tools/rag_retrieve.py`
- `backend/app/services/agentic_rag/nodes.py` (merge_node)
...
**Effort:** Medium. **Impact:** High (recall improvement). **Risk:** Low (additive, original query preserved).

1. Add `SYNONYM_EXPANSION_PROMPT` to `prompts.py` — asks LLM for corrected_query, synonyms (2-5), and negated terms.
2. Add `_expand_synonyms(query, ctx) -> (corrected, synonyms, excluded)` to `rag_retrieve.py` — cached per query string.
3. Modify `exact_search_docs` and `sparse_search_docs` in `retrieval.py` to accept `extra_queries: list[str]` — run them in parallel and RRF-fuse results.
4. In `_run_retrieval_pass`, call `_expand_synonyms` once, pass synonyms as `extra_queries` to exact and sparse legs.
5. Merge negated terms from `_expand_synonyms` with regex-extracted terms from Phase 1.
6. Unit test: "bieröffner" → corrected="Flaschenöffner", synonyms=["Flaschenöffner"], both terms searched.
...
1. Create `backend/app/services/agentic_rag/kb_profile.py` with `profile_kb(org_id, kb_ids, db, redis) -> dict`.
2. Profile includes: doc count, distinct titles (top 20), content types, date range, avg chunk length, rerank_chars adjustment.
3. Cache in Redis with TTL (default 1 hour), keyed by `kb_profile:{org_id}:{hash(kb_ids)}`.
4. In `load_context_node`, call `_load_kb_profile` — read from cache or compute synchronously (fast MySQL query).
5. Include profile summary in plan and think prompts.
6. E2E test: open chat with linked KB → profile is computed and cached → second chat open uses cache.
...
- `KB_PROFILE_ENABLED` (default true)

### Phase 5: Answer suggestions + follow-up queries (Gap 6)
**Effort:** Medium-High (touches DB, API, frontend). **Impact:** High (user-facing improvement). **Risk:** Low (additive, no graph topology change).

1. Extend `EVALUATION_SYSTEM_PROMPT` to ask for `suggestion`, `retry_strategy`, `follow_up_queries`.
2. Extend `AnswerEvaluation` dataclass with the new fields.
3. Extend `answer_evaluation_node` to extract new fields into state.
4. Add DB columns: `answer_suggestion`, `retry_strategy`, `follow_up_queries`.
5. Add to `MessageResponse` schema.
6. `save_memory_node` saves the new fields.
7. Frontend: render `follow_up_queries` as clickable `Suggestion` chips below the answer.
8. Frontend: show `suggestion` text in `ConfidenceCollapsible`.
9. Frontend: show `retry_strategy` as a label prefix on the suggestion row.
10. Wire `Suggestion.onClick` to `onFollowUp` (existing handler).

**Files:**
- `backend/app/services/agentic_rag/prompts.py`
- `backend/app/services/agentic_rag/evaluator.py`
- `backend/app/services/agentic_rag/nodes.py`
- `backend/app/services/agentic_rag/graph_state.py`
- `backend/app/services/agentic_rag/agent_graph/finalization.py`
- `backend/app/models/chat.py`
- `backend/app/schemas/chat.py`
- `frontend/src/components/chat/answer.tsx`
- `frontend/src/app/dashboard/chat/[id]/page.tsx`
- `frontend/src/types/` (message type)
- `backend/tests/test_answer_suggestions.py` (new)
- `frontend/src/components/chat/__tests__/suggestions.test.tsx` (new)

**New settings:**
- `ANSWER_SUGGESTIONS_ENABLED` (default true)
- `MAX_FOLLOW_UP_QUERIES` (default 3)

### Phase 6: Query intent extraction — filters/sort suggestions (NEW)
...
The think_node LLM discovers `filters` and `sort` fields exist from the auto-rendered schema, but neither `PLAN_SYSTEM_PROMPT` nor `THINK_SYSTEM_PROMPT` mentions them. The `Plan`/`Subtask` schema has no fields for filter suggestions. The LLM has to infer from field descriptions alone that "latest weekly update" implies `filters={"title_contains":"Weekly Update"}` and `sort={"field":"created_at","direction":"desc"}`.

This is unreliable. The LLM is already juggling plan, observations, history, and tool selection. Filter inference is an afterthought.

#### What retrievalagent does

`_adetect_filter_intent` runs in `prepare_node`, before any search:
1. Samples the collection's filterable fields + distinct values.
2. Shows them to the LLM alongside the query.
3. LLM returns a structured `FilterIntent(field, value, operator)`.
4. The filter is applied automatically to the search.

#### Recommendation: Add a query intent extraction step between rewrite and plan

Add a new `extract_intent_node` that runs after `rewrite_query_node` and before `plan_node`. It:

1. Takes the rewritten query + KB profile (from Phase 4) as input.
2. Makes one LLM call with a focused prompt: "Given this query and these available metadata fields, should any filters or sort be applied to narrow the search?"
3. Returns a structured `QueryIntent` with:
   - `suggested_filters: dict | null` — same schema as rag_retrieve's `filters`
   - `suggested_sort: dict | null` — same schema as rag_retrieve's `sort`
   - `suggested_legs: list[str] | null` — "exact","sparse" for literal lookups, null for default
   - `reasoning: str` — why these suggestions were made
4. The suggestions are stored in `AgentState.query_intent`.
5. The suggestions are injected into:
   - `plan_node` user prompt: "Suggested filters: {...}. Suggested sort: {...}."
   - `think_node` user prompt: "Suggested filters: {...}. Suggested sort: {...}. Use these when calling rag_retrieve unless you have a reason not to."
6. The think_node LLM still decides whether to use them — suggestions are non-binding. If the LLM ignores them, that's its choice. But the LLM now has explicit guidance instead of having to infer from schema descriptions.

#### Why a separate node, not folded into rewrite_query_node

- `rewrite_query_node` is about pronoun/reference resolution. Its prompt explicitly forbids adding metadata terms. Mixing filter extraction into it would require removing that guard, which could cause the rewriter to hallucinate metadata constraints.
- A separate node with a focused prompt is more reliable. The LLM has one job: detect filter/sort intent. It sees the KB profile (available fields, sample values) and the query. It doesn't need to juggle conversation history or pronoun resolution.
- The separate node can be cached per (query, kb_ids) — if the same query is retried, the intent extraction doesn't need to re-run.

#### Why non-binding suggestions, not automatic application

- The think_node LLM has full context (plan, observations, history). It might know that a suggested filter is wrong — e.g. the user said "latest weekly update" but the plan determined this is a follow-up about a specific weekly update already discussed.
- Automatic application would bypass the agent's autonomy. The agent loop exists precisely to let the LLM make informed tool-call decisions.
- If the LLM ignores a good suggestion, that's a prompt engineering problem (make the suggestion more prominent), not an architecture problem.

#### Graph topology change

```
Current:  load_context → expand_query → rewrite_query → plan → ...
New:      load_context → expand_query → rewrite_query → extract_intent → plan → ...
```

One new edge: `rewrite_query → extract_intent`. One new node: `extract_intent`.
...
```
You are a query intent extractor for a document retrieval system. Given a user query and the available metadata fields in the knowledge base, decide whether any filters or sort order should be applied to narrow the search.

Available metadata fields:
{kb_profile_fields}

Sample document titles: {sample_titles}
...
1. Suggest filters ONLY when the query clearly implies a metadata constraint:
   ...
   - "document titled X" → filters={"title_contains":"X"}
...
4. If no filters/sort/legs are implied, return null for all fields.
5. Do NOT invent field names — use only the fields listed above.

Output JSON: {"suggested_filters": {...}|null, "suggested_sort": {...}|null, "suggested_legs": [...]|null, "reasoning": "..."}
```

#### Files to change

- `backend/app/services/agentic_rag/schemas.py` — add `QueryIntent` model.
- `backend/app/services/agentic_rag/prompts.py` — add `INTENT_EXTRACTION_PROMPT`.
- `backend/app/services/agentic_rag/agent_graph/extract_intent.py` (new file) — `extract_intent_node(state, ctx)`.
- `backend/app/services/agentic_rag/agent_graph/build.py` — add node + edge.
...
**New settings:**
- `INTENT_EXTRACTION_ENABLED` (default true)
- `INTENT_EXTRACTION_CACHE_TTL` (default 300 seconds — short, queries vary)

#### Interaction with other phases

- **Phase 4 (KB profiling):** the KB profile provides the available fields and sample values that `extract_intent_node` shows to the LLM. Without Phase 4, intent extraction would need to call `kb_metadata` first (extra latency). With Phase 4, the profile is already in state.
...
1. **Phase 1** (negation extraction) — no dependencies
2. **Phase 4** (KB profiling) — no dependencies, needed by Phase 6
3. **Phase 6** (query intent extraction) — depends on Phase 4 for KB profile
4. **Phase 2** (synonym expansion) — builds on Phase 1 (merges negated terms)
5. **Phase 3** (conditional dense leg) — independent, benefits from Phases 2 + 6
6. **Phase 7** (prompt updates) — independent, do anytime
7. **Phase 5** (answer suggestions) — independent, but do last because it touches DB + frontend
...
4. Run full backend suite — must not regress.
...
|-------|-----------|----------------|----------------------|
| 1 (negation) | No | No | No — internal to rag_retrieve, agent doesn't see it |
| 2 (synonyms) | No | No | No — internal to rag_retrieve. But update `THINK_SYSTEM_PROMPT` rag_retrieve query rules: remove "Do NOT add synonyms" rule since rag_retrieve now does synonym expansion internally. The agent should still not add synonyms manually (the tool does it), but the rule's rationale changes. |
...
| 5 (answer suggestions) | No | No | No — evaluator extends its output, agent doesn't call evaluator as a tool. The `EVALUATION_SYSTEM_PROMPT` changes but that's not an agent-facing prompt. |
| 6 (query intent extraction) | No (new node, not a tool) | Yes — new `QueryIntent` model | **Yes** — `INTENT_EXTRACTION_PROMPT` is new. Plan and think prompts include intent suggestions. |
| 7 (prompt updates) | No | No | **Yes** — this IS the prompt change. |

### Specific prompt edits

**`THINK_SYSTEM_PROMPT` changes (Phases 2 + 3 + 6):**
...
3. **Already-sorted results:** when `sort={"field":"created_at","direction":"desc"}` is applied, the user explicitly wants chronological order. Reranking by relevance overrides that order. Currently our pipeline sorts after merge but before reranking, so reranking overrides the sort. This is a bug — see below.
...
Current bug: `_run_retrieval_pass` applies sort after merge but before reranking, so reranking overrides the sort. The user asks for "latest weekly update" with `sort={"field":"created_at","direction":"desc"}`, and the reranker reorders by relevance, destroying the chronological order.

### Fix: decouple scoring from ordering

New flow in `_run_retrieval_pass`:

```
1. merge_node (dedup + semantic dedup) → merged docs
2. reranking_node:
   - Score ALL docs with cross-encoder (always — quality gate)
   - Filter by reranker threshold (always — removes irrelevant docs)
   - Return scored+filtered docs, ordered by reranker score (default)
3. Final ordering:
   - If sort provided → re-apply sort to the filtered set (user's explicit order wins)
   - If no sort → keep reranker score order (relevance, current behavior)
```

This handles all cases:

| Case | Score+filter? | Final order |
|------|---------------|-------------|
| No sort, normal query | Yes | Reranker score (relevance) |
| Sort by created_at desc | Yes | created_at desc (chronological — user intent preserved) |
| Sort by created_at, all docs irrelevant | Yes → 0 pass | (empty — quality gate worked) |
| ≤2 docs, no sort | Yes | Reranker score (harmless) |
| ≤2 docs, with sort | Yes | Sort order |
| Exact match, 1 doc | Yes | Single doc (order irrelevant) |
...
**`reranking_node` (nodes.py):** no change — it already scores with `score_threshold=-inf` and returns all scored docs. The filtering happens in `filter_node`.

**`_run_retrieval_pass` (rag_retrieve.py):** move the sort step from before reranking to after filtering:

```python
# Current (buggy):
state.update(merge_node(...))
if sort:
    state["retrieved_docs"] = _sort_merged_docs(merged, sort)  # sort before rerank
state.update(reranking_node(state))  # rerank overrides sort
state.update(filter_node(state, threshold=...))  # filter by score

# Fixed:
state.update(merge_node(...))
state.update(reranking_node(state))  # score all docs
state.update(filter_node(state, threshold=...))  # filter by score (quality gate)
if sort:
    scored = state.get("retrieved_docs", [])
    state["retrieved_docs"] = _sort_merged_docs(scored, sort)  # sort AFTER filter
```

**New skip condition in `_run_retrieval_pass`:** if `legs == ["exact"]` and `sort` is provided, skip `reranking_node` + `filter_node` entirely, just sort the merged docs. This is the only case where the quality gate is safely skippable — exact FTS matches are already high-precision.
...
- `backend/app/services/agentic_rag/nodes.py` — no change to `reranking_node` or `filter_node` (they already do the right thing individually).

**No prompt changes needed** — this is internal optimization, the agent doesn't need to know.

---

## Synonym expansion: parallel BM25 vs appended to main query
...
**RRF fusion:** we already have RRF in `merge_node` for combining legs. The same RRF logic applies within each leg — fuse the per-term result lists by rank position.
