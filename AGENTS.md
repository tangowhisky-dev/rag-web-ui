...
- For agentic pipeline debugging/eval use `scripts/agentic_pipeline_eval.py` (HTTP-only; run inside `rag-web-ui-backend-1` via `docker exec` — `docker cp` the script first; `--kb-id N` reuses an already-tagged KB). Passing `"debug": true` in the chat message body streams `type="debug"` timeline events carrying stage internals (tool args, tool I/O, think/finalize prompts) — off by default, the frontend ignores them.
- Document lifecycle metadata (`document_status`, `effective_from/to`, `version`, `owner`) is set via `PUT /api/knowledge-base/{kb}/documents/{doc}/markdown` with unchanged markdown — that is the metadata-only update path and does not trigger re-ingestion.
- Future optimization (not yet applied): `_dense_search`/`_sparse_search` in `backend/app/services/retrieval/retrieval.py` call Qdrant with `with_vectors=True` to populate `metadata._dense_vector` for `semantic_dedup`. Only the `/search` endpoint (`_run_retrieval_legs` in `api_v1/search.py`) actually consumes it — agentic tools dedup by content hash and `rerank_results` whitelists metadata (dropping the vector, so its `semantic_dedup` is already a no-op). If payload size matters, add an `include_vectors: bool = False` param to `dense_search_docs`/`sparse_search_docs`, pass `with_vectors=include_vectors` to Qdrant, and set `include_vectors=True` only in `search.py`.
