...
export default function KBDocumentEditorPage() {
  ...
  const loadMarkdown = useCallback(async () => {
    ...
    try {
      const resp = (await api.get(
        `/api/knowledge-base/${kbId}/documents/${docId}/markdown`,
      )) as MarkdownResponse;
  ...
  const pollStatus = useCallback(async () => {
    try {
      const resp = (await api.get(
        `/api/knowledge-base/${kbId}/documents/${docId}/ingest-status`,
  ...
  }, [pendingNavigation, router, kbId]);

  // Save — triggers immediate re-ingestion (delete old chunks + graph, re-ingest from saved markdown)
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Re-ingesting with fresh chunks and graph.",
      });
      // Start polling for re-ingestion progress
      if (!pollRef.current) {
        pollStatus();
        pollRef.current = setInterval(pollStatus, 2000);
      }
  ...
  }, [markdown, lockVersion, kbId, docId, toast, pollStatus]);
